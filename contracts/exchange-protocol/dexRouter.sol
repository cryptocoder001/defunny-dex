...
contract PancakeswapRouter is IPancakeswapRouter {
    ...
    function _addLiquidity(
        ...
        if (
            IPancakeswapFactory(factory).getPair(tokenA, tokenB) == address(0)
        ) {
            IPancakeswapFactory(factory).createPair(tokenA, tokenB);
        }
        (uint256 reserveA, uint256 reserveB) = PancakeswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = PancakeswapLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "PancakeswapRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = PancakeswapLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "PancakeswapRouter: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        (uint256 amount0, uint256 amount1) = IPancakeswapPair(pair).burn(to);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ...
                : _to;
            IPancakeswapPair(PancakeswapLibrary.pairFor(factory, input, output))
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(
            ...
            "PancakeswapRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
        ...
        returns (uint256[] memory amounts)
    {
        require(
            ...
            "PancakeswapRouter: INVALID_PATH"
        );
        amounts = PancakeswapLibrary.getAmountsIn(factory, amountOut, path);
        require(
            amounts[0] <= amountInMax,
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IPancakeswapPair pair = IPancakeswapPair(
                PancakeswapLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = PancakeswapLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
...
library PancakeswapLibrary {
    ...
    function quote(
        ...
        require(
            ...
            "PancakeswapLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountIn(
        ...
        require(
            amountOut > 0,
            "PancakeswapLibrary: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        require(
            reserveIn > 0 && reserveOut > 0,
            "PancakeswapLibrary: INSUFFICIENT_LIQUIDITY"
